...
/// Generator-owned outputs must also carry a `DO NOT EDIT` or `Generated by:` header.
pub fn generated_zone_existence_hits(ctx: &AuditContext) -> Vec<FindingHit> {
    ...
    for zone in &file.zone {
        ...
        let reviewed_source = write_policy == "reviewed_manual";
        let full = ctx.root.join(path);
        if !full.exists() {
        ...
        // Reviewed-manual zones are source artifacts governed by review, not generated output.
        // Other readable zones need a text header or format-native generated identity.
        if !reviewed_source {
...
/// Write policies that permit changes outside a generator-only workflow.
/// Auditor output and lockfiles are refreshed by their owning tools; reviewed-manual
/// zones are source artifacts changed under ordinary review.
const GENERATED_ZONE_REVIEWED_WRITE_POLICIES: &[&str] =
    &["auditor_output", "lockfile", "reviewed_manual"];
...
/// `auditor_output`, `lockfile`, or `reviewed_manual`).
///
...
/// generator-managed. Reuses the same manifest read path as [`generated_zone_paths`]
/// but keeps only the zones that the governance guard should police.
fn governed_generated_zone_paths(ctx: &AuditContext) -> Vec<String> {
    ...
    file.zone
        .into_iter()
        .filter(|zone| !GENERATED_ZONE_REVIEWED_WRITE_POLICIES.contains(&zone.write_policy.trim()))
...
"#
    ...
    #[test]
    fn reviewed_manual_zone_does_not_require_generated_header() {
        let dir = tempdir().unwrap();
        let root = dir.path();
        ...
        std::fs::write(
            ...
            r#"[[zone]]
path = "contracts/model-artifact-v2.json"
source = "crates/model/src/artifact.rs"
...
"#,
        ...
        .unwrap();
        std::fs::write(
            root.join("contracts/model-artifact-v2.json"),
            "{\"version\":2}\n",
        )
        .unwrap();

        let hits = generated_zone_existence_hits(&zone_edit_ctx(root.to_path_buf()));
        assert!(
            hits.is_empty(),
            "reviewed source must not need a generated comment header: {hits:?}"
        );
    }

    #[test]
    fn reviewed_manual_zone_still_requires_declared_file() {
        let dir = tempdir().unwrap();
        let root = dir.path();
        std::fs::create_dir_all(root.join("agent")).unwrap();
        std::fs::write(
...
"#,
        ...
        let hits = generated_zone_existence_hits(&zone_edit_ctx(root.to_path_buf()));
        assert_eq!(hits.len(), 1, "missing reviewed source must fail closed");
        assert_eq!(
            hits[0].matched_term.as_deref(),
            Some("missing-generated-zone-file")
        );
    }
...
"#,
        ...
        .unwrap();
        std::fs::write(root.join("generated/client.json"), "{\"version\":2}\n").unwrap();

        let hits = generated_zone_existence_hits(&zone_edit_ctx(root.to_path_buf()));
        assert_eq!(
            hits.len(),
            1,
            "generator-owned output without identity must fail closed"
        );
        assert_eq!(
            hits[0].matched_term.as_deref(),
            Some("missing-generated-header")
        );
    }

    #[test]
    fn generated_zone_edit_allows_reviewed_manual_source_change() {
        let dir = tempdir().unwrap();
        let root = dir.path();
        init_git_repo(root);
        std::fs::create_dir_all(root.join("agent")).unwrap();
...
"#,
        ...
        assert!(
            hits.is_empty(),
            "reviewed-manual source changes are governed by review: {hits:?}"
        );
    }
